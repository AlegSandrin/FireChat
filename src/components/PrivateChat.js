...
export default function PrivateChat({ setShowAlert }) {
  ...
  const querySnapshot = useChat((state) => state.privateChat?.[docId]?.docs);
  const snap = useChat((state) => state.privateChat?.[docId]?.docChanges());
  ...
  useEffect(() => {
  let docsData = [];
    snap && snap?.forEach((docs) => {
      docsData.push(docs.doc.data());
    });
  ...
  const uploadFile = () => {
    ...
    const fileRef = ref(
      storage,
      `privateChat/${docId}/${file.name + uuidv4()}`
    );
    uploadBytes(fileRef, file).then((promise) => {
      getDownloadURL(promise.ref).then((url) => {
        ...
        firestore
          ...
          .collection("messages")
          .add(addFile);
        firestore.collection("privateChat").doc(docId).update({
          lastMessage: firebase.firestore.FieldValue.serverTimestamp(),
        });
      });
    });
  ...
  const fileValidation = (file) => {
    ...
    if (file.target.files[0]?.size > sizelimit) {
      ...
      file.target.files[0]?.type.includes("image") ||
